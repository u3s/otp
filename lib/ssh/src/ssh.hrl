...
%% logging macros to be used in ssh code
%% intention is to generate  similar events as error_logger
-define(SSH_ERROR_MSG(FMT, ARGS), ?SSH_MSG(error, FMT, ARGS, error_msg, error)).
-define(SSH_NOTICE_MSG(FMT, ARGS), ?SSH_MSG(notice, FMT, ARGS, info_msg, info_msg)).
-define(SSH_MSG(LEVEL, FMT, ARGS, LABEL, TAG),
            logger:log(LEVEL,
                       #{label => {error_logger, LABEL},
                         format => FMT,
                         args => ARGS},
                       #{error_logger => #{tag => TAG},
                         report_cb => ?REPORT_TO_FORMAT_FUN})).

-define(SSH_ERROR_REPORT(FMT, ARGS), logger:log(error, ?ERROR_REPORT_FUN(FMT), ARGS)).
-define(SSH_WARNING_REPORT(FMT, ARGS), logger:log(warning, ?WARNING_REPORT_FUN(FMT), ARGS)).
%% notice user with INFO_REPORT_FUN because that is how error_logger behaves
-define(SSH_NOTICE_REPORT(FMT, ARGS), logger:log(notice, ?INFO_REPORT_FUN(FMT), ARGS)).
%% help macros
-define(ERROR_REPORT_FUN(FMT), ?REPORT_FUN(FMT, error_report, std_error, error_report)).
-define(WARNING_REPORT_FUN(FMT), ?REPORT_FUN(FMT, warning_report, std_warning, warning_report)).
-define(INFO_REPORT_FUN(FMT), ?REPORT_FUN(FMT, info_report, std_info, info_report)).
%% macro below retains the original log event structure generated by error_logger
-define(REPORT_FUN(FMT, TAG, TYPE, FUNC),
        begin
            fun(__Args) ->
                    __Report =
                        #{label => {error_logger, FUNC},
                          report =>
                              %% FIXME should flatten be called here?
                              lists:flatten(io_lib:format(FMT, __Args))},
                    __Meta =
                        #{error_logger =>
                              #{tag => TAG, type => TYPE},
                          report_cb => ?REPORT_TO_FORMAT_FUN},
...
%% -----------------------------------------------------------------
-define(REPORT_TO_FORMAT_FUN,
        begin
            fun(#{label:={error_logger,_},
                  report:=___Report}) when is_map(___Report) ->
               ...
               (#{label:={error_logger,_},
                  ...
                  args:=___Args}) ->
                    ...
                    try io_lib:scan_format(___Format,___Args) of
                        _ -> {___Format,___Args}
                    catch _:_ ->
                            {"ERROR: ~tp - ~tp",[___Format,___Args]}
                    end;
               (___Term) ->
                    logger:format_otp_report(___Term)
            end
        end).
