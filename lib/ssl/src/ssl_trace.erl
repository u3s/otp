...
trace_profiles() ->
    [{api,
     ...
     {csp, %% OCSP
      fun(M, F, A) -> dbg:tpl(M, F, A, x) end,
      fun(M, F, A) -> dbg:ctpl(M, F, A) end,
      [{openssl_ocsp_SUITE, [{start_ocsp_responder, 2},
                             {ocsp_responder_init, 3},
                             {init_per_testcase, 2},
                             {ocsp_responder_loop, 2}]},
       ...
       {ssl, [{opt_ocsp, 3}]},
       {ssl_certificate, [{cert_status_check, 5}]},
       ...
       {tls_handshake, [{ocsp_expect, 1}]}]},
     {crt, %% certificates
      fun(M, F, A) -> dbg:tpl(M, F, A, x) end,
      fun(M, F, A) -> dbg:ctpl(M, F, A) end,
      [{ssl_certificate, [{validate, 3}, {verify_cert_extensions, 2}]}]},
     {kdt, %% key update
      fun(M, F, A) -> dbg:tpl(M, F, A, x) end,
      fun(M, F, A) -> dbg:ctpl(M, F, A) end,
